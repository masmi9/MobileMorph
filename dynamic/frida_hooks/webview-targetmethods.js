Java.perform(function () {
    ...
    // Filter on "risky" WebView methods
    var targetMethods = ["getUrl", "loadUrl", "loadData", "postUrl", "addJavascriptInterface", "evaluateJavascript", "setWebContentsDebuggingEnabled"];
    // Iterate over each method
    targetMethods.forEach(function (method) {
        //var methodName = method.getName();
        console.log("[+] Hooking WebView." + method);
        var overloads = WebView[method].overloads;
        ...
        overloads.forEach(function (overload) {
            overload.implementation = function () {
                ...
                if (arguments.length > 0) logOutput("[>] WebView." + method + " called with: [" + argsString + "]");
                ...
                try {
                    if (returnValue != null && returnValue != undefined) logOutput("[<] WebView." + method + " returned: " + returnValue.toString());
                } catch (e) {
                    logOutput("[<] WebView." + method + " returned: [unreadable]");
                }
